<div class="mobile-header js-mobile-menu">
    <button class="mobile-header__btn js-mobile-menu-btn">
        <span class="mobile-header__btn-inner"></span>
    </button>

    <a href="/" class="mobile-header__logo">
        <svg role="img" aria-label="Alpha Capital Education" width="182" height="26">
            <use xlink:href="#logo-current-color"></use>
        </svg>
    </a>

    <div class="mobile-header__right">
        <a href="/register/profile">
            <svg role="img" width="20" height="20">
                <use xlink:href="#ico-user"></use>
            </svg>
        </a>

        <a href="/catalog/cart">
            <svg role="img" width="16" height="20">
                <use xlink:href="#ico-basket"></use>
            </svg>
        </a>
    </div>

    <div class="mobile-menu">
        <div class="mobile-menu__menu js-accordion">
            <div class="mobile-menu__menu-item js-accordion-item ">
                <a class="mobile-menu__link" href="/catalog">Kурсы</a>
            </div>

            <div class="mobile-menu__menu-item js-accordion-item ">
                <button class="mobile-menu__link js-accordion-btn" type="button">Школы</button>

                <ul class="mobile-menu__content js-accordion-content">
                    {section name=i loop=$m_school_1}
                        <li><a href="/services/{$m_school_1[i].ALIAS}">{$m_school_1[i].TITLE}</a></li>
                    {/section}
                </ul>
            </div>

            <div class="mobile-menu__menu-item js-accordion-item ">
                <button class="mobile-menu__link js-accordion-btn" type="button">Университеты</button>

                <ul class="mobile-menu__content js-accordion-content">
                    {section name=i loop=$m_school_2}
                        <li><a href="/services/{$m_school_2[i].ALIAS}">{$m_school_2[i].TITLE}</a></li>
                    {/section}
                </ul>
            </div>

            <div class="mobile-menu__menu-item js-accordion-item ">
                <a class="mobile-menu__link" href="/services/opekunstvo-p">Опекунство</a>
            </div>



            <div class="mobile-menu__menu-item js-accordion-item ">
                <button class="mobile-menu__link js-accordion-btn" type="button">Другие услуги</button>

                <ul class="mobile-menu__content js-accordion-content">
                    {section name=i loop=$m_school_4}
                        <li><a href="/services/{$m_school_4[i].ALIAS}">{$m_school_4[i].TITLE}</a></li>
                    {/section}
                </ul>
            </div>
        </div>

        <ul class="mobile-menu__footer">
            <li><a href="">Главная</a></li>
            <li><a href="">Контакты</a></li>
            <li><a href="">О компании</a></li>
            <li><a href="">T&amp;C</a></li>
        </ul>

        <ul class="mobile-menu__social">
            <li>
                <a class="mobile-menu__social-link" href="" title="Alpha Capital Education Instagram" aria-label="Alpha Capital Education Instagram">
                    <svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 511 511.9" width="20" height="20">
                        <path d="M510.95 150.5c-1.2-27.2-5.598-45.898-11.9-62.102-6.5-17.199-16.5-32.597-29.6-45.398-12.802-13-28.302-23.102-45.302-29.5-16.296-6.3-34.898-10.7-62.097-11.898C334.648.3 325.949 0 256.449 0s-78.199.3-105.5 1.5c-27.199 1.2-45.898 5.602-62.097 11.898-17.204 6.5-32.602 16.5-45.403 29.602-13 12.8-23.097 28.3-29.5 45.3-6.3 16.302-10.699 34.9-11.898 62.098C.75 177.801.449 186.5.449 256s.301 78.2 1.5 105.5c1.2 27.2 5.602 45.898 11.903 62.102 6.5 17.199 16.597 32.597 29.597 45.398 12.801 13 28.301 23.102 45.301 29.5 16.3 6.3 34.898 10.7 62.102 11.898 27.296 1.204 36 1.5 105.5 1.5s78.199-.296 105.5-1.5c27.199-1.199 45.898-5.597 62.097-11.898a130.934 130.934 0 0074.903-74.898c6.296-16.301 10.699-34.903 11.898-62.102 1.2-27.3 1.5-36 1.5-105.5s-.102-78.2-1.3-105.5zm-46.098 209c-1.102 25-5.301 38.5-8.801 47.5-8.602 22.3-26.301 40-48.602 48.602-9 3.5-22.597 7.699-47.5 8.796-27 1.204-35.097 1.5-103.398 1.5s-76.5-.296-103.403-1.5c-25-1.097-38.5-5.296-47.5-8.796C94.551 451.5 84.45 445 76.25 436.5c-8.5-8.3-15-18.3-19.102-29.398-3.5-9-7.699-22.602-8.796-47.5-1.204-27-1.5-35.102-1.5-103.403s.296-76.5 1.5-103.398c1.097-25 5.296-38.5 8.796-47.5C61.25 94.199 67.75 84.1 76.352 75.898c8.296-8.5 18.296-15 29.398-19.097 9-3.5 22.602-7.7 47.5-8.801 27-1.2 35.102-1.5 103.398-1.5 68.403 0 76.5.3 103.403 1.5 25 1.102 38.5 5.3 47.5 8.8 11.097 4.098 21.199 10.598 29.398 19.098 8.5 8.301 15 18.301 19.102 29.403 3.5 9 7.699 22.597 8.8 47.5 1.2 27 1.5 35.097 1.5 103.398s-.3 76.301-1.5 103.301zm0 0"></path>
                        <path d="M256.45 124.5c-72.598 0-131.5 58.898-131.5 131.5s58.902 131.5 131.5 131.5c72.6 0 131.5-58.898 131.5-131.5s-58.9-131.5-131.5-131.5zm0 216.8c-47.098 0-85.302-38.198-85.302-85.3s38.204-85.3 85.301-85.3c47.102 0 85.301 38.198 85.301 85.3s-38.2 85.3-85.3 85.3zm0 0M423.852 119.3c0 16.954-13.747 30.7-30.704 30.7-16.953 0-30.699-13.746-30.699-30.7 0-16.956 13.746-30.698 30.7-30.698 16.956 0 30.703 13.742 30.703 30.699zm0 0"></path>
                    </svg>
                </a>
            </li>
            <li>
                <a class="mobile-menu__social-link" href="" title="Alpha Capital Education Skype" aria-label="Alpha Capital Education Skype">
                    <svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 438.536 438.536" width="20" height="20">
                        <path d="M415.7 262.093c3.039-14.657 4.564-28.935 4.564-42.826 0-27.218-5.276-53.247-15.844-78.087-10.562-24.838-24.838-46.249-42.825-64.237-17.987-17.988-39.395-32.263-64.234-42.829-24.845-10.559-50.866-15.843-78.088-15.843-13.894 0-28.171 1.524-42.827 4.57C156.651 7.614 134.381 0 109.637 0 79.375 0 53.538 10.705 32.124 32.115 10.708 53.531.004 79.368.004 109.631c0 24.744 7.614 47.014 22.839 66.809-3.044 14.655-4.568 28.933-4.568 42.827 0 27.215 5.28 53.243 15.843 78.085 10.562 24.838 24.838 46.25 42.827 64.241 17.987 17.986 39.401 32.257 64.239 42.824 24.84 10.564 50.869 15.845 78.087 15.845 13.893 0 28.17-1.526 42.834-4.572 19.79 15.229 42.058 22.847 66.804 22.847 30.259 0 56.103-10.711 77.505-32.12 21.416-21.416 32.12-47.253 32.12-77.519-.005-24.74-7.616-47.007-22.834-66.805zm-90.228 42.397c-6.276 11.136-14.702 20.033-25.263 26.696-10.567 6.663-22.224 11.748-34.975 15.273-12.751 3.518-26.073 5.283-39.971 5.283-32.163 0-59.855-6.235-83.078-18.705-23.223-12.471-34.833-27.453-34.833-44.968 0-8.562 2.428-15.693 7.282-21.408 4.853-5.712 12.038-8.562 21.555-8.562 5.52 0 10.657 1.522 15.415 4.564 4.758 3.046 9.135 6.715 13.134 10.999 3.999 4.285 8.326 8.562 12.99 12.847 4.661 4.285 10.847 7.946 18.555 10.992 7.71 3.046 16.418 4.572 26.128 4.572 12.371 0 22.36-2.423 29.981-7.275 7.61-4.859 11.416-10.999 11.416-18.418 0-7.618-3.042-13.326-9.13-17.132-4.182-2.673-14.846-6.098-31.977-10.283l-41.688-10.284c-11.419-2.662-21.222-5.752-29.408-9.271-8.186-3.524-15.8-8.134-22.841-13.849-7.039-5.708-12.369-12.891-15.986-21.555-3.616-8.658-5.424-18.796-5.424-30.406 0-13.896 3.189-26.121 9.563-36.688 6.374-10.565 14.849-18.846 25.409-24.841 10.562-5.996 21.935-10.468 34.119-13.418 12.179-2.951 24.742-4.426 37.685-4.426 18.276 0 35.589 2.19 51.961 6.567 16.368 4.377 29.882 10.801 40.538 19.271 10.657 8.473 15.985 17.942 15.985 28.409 0 8.562-2.707 15.893-8.138 21.982-5.414 6.088-12.601 9.134-21.55 9.134-4.948 0-9.514-1.143-13.702-3.427a54.891 54.891 0 01-11.423-8.278 217.22 217.22 0 00-10.852-9.563c-3.806-3.14-8.946-5.852-15.41-8.136-6.479-2.284-13.802-3.427-21.986-3.427-27.406 0-41.112 7.323-41.112 21.982 0 3.234.715 6.09 2.141 8.564 1.427 2.474 3 4.473 4.71 5.996 1.709 1.525 4.565 3.046 8.564 4.57 3.999 1.525 7.33 2.622 9.994 3.284 2.663.666 6.854 1.665 12.562 2.996l29.694 6.851c9.325 2.096 17.648 4.286 24.981 6.567 7.331 2.288 15.181 5.521 23.555 9.71 8.374 4.187 15.321 8.848 20.838 13.988 5.523 5.144 10.137 11.656 13.853 19.555 3.714 7.901 5.564 16.61 5.564 26.124-.001 13.894-3.136 26.411-9.425 37.544z"></path>
                    </svg>
                </a>
            </li>
            <li>
                <a class="mobile-menu__social-link" href="tel:+447481515012" title="Alpha Capital Education Phone" aria-label="Alpha Capital Education Phone">

                    <svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 18 18" width="20" height="20">
                        <path d="M9.032 0C4.085 0 .065 3.998.065 8.917c0 1.779.576 3.408 1.476 4.801L0 18l4.796-1.345c1.272.691 2.684 1.179 4.236 1.179 4.948 0 8.968-3.998 8.968-8.917C18 3.997 13.98 0 9.032 0zm0 1.372c4.198 0 7.588 3.37 7.588 7.545 0 4.174-3.39 7.545-7.588 7.545a7.584 7.584 0 01-3.923-1.093l-.253-.155-2.657.739.862-2.38-.21-.289a7.48 7.48 0 01-1.407-4.367c0-4.175 3.39-7.545 7.588-7.545zM5.67 4.116c-.156 0-.415.064-.635.321-.216.252-.83.868-.83 2.122 0 1.249.851 2.46.97 2.631.118.172 1.676 2.739 4.058 3.843.566.262 1.008.417 1.352.535.572.193 1.09.166 1.499.102.458-.075 1.406-.616 1.606-1.21.194-.596.199-1.105.134-1.212-.053-.107-.215-.171-.452-.295-.238-.128-1.407-.745-1.623-.825-.215-.091-.377-.129-.538.123-.157.258-.61.83-.75.997-.14.171-.274.193-.512.064-.242-.128-1.007-.396-1.913-1.264-.706-.676-1.185-1.506-1.32-1.763-.14-.258-.016-.392.102-.515.108-.118.237-.3.356-.45.119-.145.156-.257.237-.423a.49.49 0 00-.016-.445c-.065-.129-.539-1.383-.733-1.892-.194-.493-.393-.428-.539-.434-.135-.01-.296-.01-.453-.01z"></path>
                    </svg>
                    <span>+447481515012</span>
                </a>
            </li>
        </ul>
    </div>
</div>