    ...
    public function ShowCart(){
        ...
        if ($cart =$this->GetCartDB()){
            ...
            if (isset($this->get['act']) && $this->get['act'] =='delete-cart' && isset($this->get['act2'])){
                ...
                $this->UpdateCartDB($cart, $_COOKIE['hash'] );
                $this->Head('/catalog/cart');
            }
        }
        ...
        if ($cart){
            foreach ($cart as &$car){
                ...
                foreach ($car->p_list as $c){
                    $p = $p + $c->price;
                    if (!property_exists($car->p_list, 'count')){
                        $c->count = 1;
                    }
    ...
    public function ShowItems(){
        ...
        if (isset($this->get['find'])){
            ...
            if (isset($this->get['country']) && $this->get['country'] > 0){
                ...
                $this->assign(array(
                    'regions' => $this->GetRegions($this->get['country'])
                ));
            }
            if (isset($this->get['region']) && $this->get['region'] > 0){
                $url .= 'region=' . $this->get['region'] . '/';
                $where[] = 'regions.REGION_ID = ' . $this->get['region'];
                $this->assign(array(
                    'cities' => $this->GetCities($this->get['region'])
                ));
            }
...
;
    ...
    public function ShowPay(){
        if (isset($_COOKIE['cart']) && $this->login){
            $params = array(
        ...
        if (isset($this->get['pay'])){
            $cart = $this->GetCart($this->get['pay']);
        } else {
            $cart = $this->GetCart();
        }
    ...
    public function CartPro(){
        ...
        foreach ($cart as &$pl){
            foreach($pl->p_list as &$p){
                if ($id == $p->id){
                    $p->count = $this->post['cart_pro'];
                }
